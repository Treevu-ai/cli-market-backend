# Hot-patch funnel filter onto the live Fly.io machine.
Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"
Set-Location $PSScriptRoot\..

function Invoke-FlyPython([string]$Code) {
    fly ssh console -a cli-market-api -C "python -c `"$Code`""
    if ($LASTEXITCODE -ne 0) { throw "fly ssh failed" }
}

function Send-File([string]$LocalPath, [string]$RemotePath) {
    $bytes = [IO.File]::ReadAllBytes((Resolve-Path $LocalPath))
    $b64 = [Convert]::ToBase64String($bytes)
    Invoke-FlyPython "open('$RemotePath','wb').close()"
    for ($i = 0; $i -lt $b64.Length; $i += 3000) {
        $len = [Math]::Min(3000, $b64.Length - $i)
        $chunk = $b64.Substring($i, $len)
        Invoke-FlyPython "import base64; open('$RemotePath','ab').write(base64.b64decode('$chunk'))"
    }
